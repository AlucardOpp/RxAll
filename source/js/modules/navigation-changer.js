export class NavigationChanger {
  ...
  _changeLinksActiveState() {
    this._linkElements.forEach((link, index) => {
      ...
      if (currentBlockElement.getBoundingClientRect().bottom - 81 > this._blockShift && currentBlockElement.getBoundingClientRect().top - 81 <= this._blockShift) {
        link.classList.add('is-active');
      } else {
        link.classList.remove('is-active');
      }

      if (document.body.getBoundingClientRect().bottom - 81 - window.innerHeight < this._blockShift && currentBlockElement.getBoundingClientRect().top - 81 > this._blockShift) {
